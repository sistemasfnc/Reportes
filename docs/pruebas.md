...
- **2026-08-02** — Sin pruebas nuevas. Cambio: guard clause de longitud de arreglo (`values.Length > 44`) antes de indexar la columna 44 en `CargarValoraciones.ValidateColumns()`/`GetPatients()`. Cae en "Páginas ASPX (code-behind WebForms)", ya excluido arriba; se verificó manualmente reproduciendo el archivo real que disparaba el `IndexOutOfRangeException`.
- **2026-08-02** — Sin pruebas nuevas. Cambio: `ProcessSanitas()` en `Facturacion2885.cs` deja de bifurcar entre `GenerateSanitasSupports()`/`GenerateSupports()` según cantidad de documentos y siempre genera un soporte único. Es lógica de negocio pura (elección de rama), pero se decidió con el usuario diferir la prueba junto con el resto de `Facturacion2885` (mismo caso que 2026-07-24, sin proyecto de tests aún).
- **2026-09-11** — Sin pruebas nuevas. Cambio: `GenerateFiles()` en `Facturacion2885.cs` agrega `try/catch` por factura (antes una excepción en una factura abortaba toda la relación) y enriquece el mensaje de error con documento/cups. Es lógica de control de flujo testeable en principio (caso feliz: sigue con la siguiente factura; caso error: el mensaje incluye documento y cups), pero se decidió con el usuario diferir la prueba una tercera vez (mismo motivo que 2026-07-24 y 2026-08-02: `Facturacion2885` está fuertemente acoplada a `Configuration` estático, Oracle vía `FacadeDesmaterializacion` y el sistema de archivos, sin proyecto de tests aún en la solución).
- **2026-09-22** — Sin pruebas nuevas. Cambio: `GenerateSpecialPlanSupports()` en `Facturacion2885.cs` agrega documento/cups al mensaje de error cuando falta un archivo HC o SOP (mismo patrón de `GenerateFiles()` del 2026-09-11). Se diferió la prueba por la misma razón acumulada desde 2026-07-24 (sin proyecto de tests en la solución). El resto del requerimiento fue documentación de un proceso externo (`FNCCargoProgramas`, repo `FNCESB`) sin lógica propia de este repo que cubrir.
