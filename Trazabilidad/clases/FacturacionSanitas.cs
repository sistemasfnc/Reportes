...
{
    public class FacturacionSanitas : IDisposable
    {
            ...
            foreach (var item in lgeneric)
            {
                ...
                if (lsupports.Count > 0)
                {
                    foreach (string support in lsupports)
                    {
                        ...
                        {
                            EventLog.LogError.WriteError("Trazabilidad", "Aplicacion", ex);                            
                        }                        
                    }
                }
        ...
        private string CreatePDFSupports(List<Generic> lgeneric, string sinvoice)
        {
            ...
            if (File.Exists(pdffile))
            {
                ...
                fs = new FileStream(pdffile + "_temp", FileMode.Create);  // Create a temporary file to avoid corruption
