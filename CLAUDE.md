...
- **`Config\Configuration.cs`** — lee desde un path **hardcodeado** (`C:\www\Pedro_Romero\Proyectos_NET\BusDatos\Reportes\Config\bin\Debug\Config.dll.config`); si no existe ahí, toda la aplicación falla al arrancar. Cambiar al migrar de máquina. Ver `arquitectura.md` centralizado para el esquema XML esperado.
